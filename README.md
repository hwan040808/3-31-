...
<head>
    ...
    <style>
        body {
            ...
            background-color: #e5e7eb; /* 데스크톱 전체 배경색 */
        ...
        /* 📱 핵심 수정: 스마트폰 목업(Mockup) UI 컨테이너 */
        .mobile-container {
            width: 100%;
            height: 100%;
            position: relative;
            background: #f9f9ff;
            overflow: hidden; /* 내부 스크롤이 프레임을 벗어나지 않게 설정 */
            display: flex;
            flex-direction: column;
        }

        /* 데스크톱(500px 이상) 화면일 때 실제 폰처럼 보이게 프레임 추가 */
        @media (min-width: 500px) {
            .mobile-container {
                width: 390px; /* 아이폰 화면 너비 */
                height: 844px; /* 아이폰 화면 높이 */
                border-radius: 44px;
                border: 14px solid #1f2937; /* 스마트폰 베젤 색상 */
                box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5), inset 0 0 0 4px #374151; /* 입체감 그림자 */
            }
...
<div class="mobile-wrapper">
    ...
    <div class="mobile-container">
        ...
        <!-- 스크롤이 발생하는 내부 영역 -->
        <div class="scroll-area custom-scrollbar pb-32">
            
            <!-- Header (스크롤 시 상단 고정) -->
            <header class="w-full sticky top-0 z-30 bg-surface/90 backdrop-blur-lg border-b border-outline-variant/20 pt-4">
        ...
        <!-- 📱 핵심 수정: 컨테이너 내부에 고정된 요소들 (화면 따라 내려가지 않음) -->
        ...
        <nav class="absolute bottom-0 left-0 w-full z-50 bg-surface/90 backdrop-blur-xl border-t border-outline-variant/20 px-4 pb-6 pt-3 flex justify-around items-center shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] rounded-t-[1.5rem]">
...
<script>
    ...
    function showTab(tabId, el) {
        ...
        setTimeout(() => {
            ...
            // 📱 핵심 수정: 화면 전체가 아닌, 폰 내부 스크롤 영역을 최상단으로 이동
    ...
    function showToast(message) {
        ...
        toast.className = 'absolute top-24 left-1/2 -translate-x-1/2 bg-on-surface text-surface px-5 py-3 rounded-xl shadow-xl z-[60] text-sm font-medium transition-all duration-300 transform -translate-y-4 opacity-0 flex items-center gap-2 max-w-[90%] w-max';
        
        toast.innerHTML = `<span class="material-symbols-outlined text-[18px]">info</span> ${message}`;
        
        // 메시지가 프레임 바깥으로 나가지 않도록 모바일 컨테이너 내부에 부착
